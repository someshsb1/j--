...
class JMethodDeclaration extends JAST implements JMember {
    ...
    public JAST analyze(Context context) {
        ...
        for (JFormalParameter param : params) {
            ...
            //method support to skip and offset for long and double
            if (param.type() == Type.DOUBLE || param.type() == Type.LONG) { this.context.nextOffset(); }
            defn.initialize();
            this.context.addEntry(param.line(), param.name(), defn);
        }

        //During the analysis of JMethodDeclaration, convert the list of exceptions (stored as TypeName objects)
        //into a list of their JVM names (stored as strings).
        if (this.exceptions != null) {
            exceptionNames = new ArrayList<String>();
            for (int i = 0; i < exceptions.size(); i++) {
                TypeName exception = exceptions.get(i);
                Type rType = (Type) exception.resolve(context);
                String jvmName = rType.jvmName();
                exceptionNames.add(jvmName);
            }
        }
    ...
    public void partialCodegen(Context context, CLEmitter partial) {
        ...
        } else if (returnType == Type.LONG) { //support for long added
            partial.addNoArgInstruction(LCONST_0);
            partial.addNoArgInstruction(LRETURN);
        } else if (returnType == Type.DOUBLE) { //support for Double added
