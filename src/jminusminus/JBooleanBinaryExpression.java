...
class JLogicalOrOp extends JBooleanBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchExpected(line(), Type.BOOLEAN); // lhs rhs both type boolean
        rhs.type().mustMatchExpected(line(), Type.BOOLEAN);
        type = Type.BOOLEAN; //operand type boolean
        return this;
    }
...
class JNotEqualOp extends JBooleanBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchExpected(line(), rhs.type());  // lhs rhs both type boolean
        type = Type.BOOLEAN;
        return this;
    }
