...
enum TokenKind {
    ...
    VOID("void"), WHILE("while"), BREAK("break"), CASE("case"), CATCH("catch"), CONTINUE(
            "continue"), DEFAULT("default"), DO("do"), DOUBLE("double"), FINALLY("finally"), FOR(
                    "for"), IMPLEMENTS("implements"), INTERFACE("interface"), LONG("long"),
    SWITCH("switch"), THROW("throw"), THROWS("throws"), TRY("try"),

    // Operators.
    ASSIGN("="), AND("&"), ALSHIFT("<<"), ARSHIFT(">>"), AND_ASSIGN("&="),
    ALSHIFT_ASSIGN("<<="), ARSHIFT_ASSIGN(">>="), COLON(":"), DEC("--"), DIV("/"),
    DIV_ASSIGN("/="), EQUAL("=="), GE(">="), GT(">"), INC("++"), LAND("&&"),
    LE("<="), LNOT("!"), LOR("||"), LT("<"), LRSHIFT(">>>"),
    LRSHIFT_ASSIGN(">>>="), MINUS_ASSIGN("-="), MINUS("-"), NOT("~"), NOT_EQUAL("!="),
    OR("|"), OR_ASSIGN("|="), PLUS("+"), PLUS_ASSIGN("+="), QUESTION("?"), REM("%"), REM_ASSIGN("%="),
    STAR("*"), STAR_ASSIGN("*="), XOR("^"), XOR_ASSIGN("^="),
