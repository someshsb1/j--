...
//            else if (have(ABSTRACT)) { // TYPO as confirmed by professor on PIAZZA @70
//                mods.add("abstract");
//                if (scannedABSTRACT) {
//                    reportParserError("Repeated modifier: abstract");
//                }
...
//            }
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        if (have(IMPLEMENTS)) {
            ...
            return new JClassDeclaration(line, mods, name, superType, interfaces, classBody());
        }
        return new JClassDeclaration(line, mods, name, superType, null, classBody());
    }
    ...
    /**
     * Parses a interface body and returns a list of interface members in the body.
     *
     * <pre>
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        // Implicitly adding "abstract" and "public" modifiers with a check condition if
        // declared or not, and if not then add.
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
        ...
        } else if (have(WHILE)) { //Support added for the While block in parser
            JExpression test = parExpression();
            JStatement statement = statement();
            return new JWhileStatement(line, test, statement);
        } else if (have(DO)) { //Support added for the Do-While block in parser
            JStatement statement = statement();
            mustBe(WHILE);
            JExpression test = parExpression();
            mustBe(SEMI);
            return new JDoStatement(line, statement, test);
        } else if (have(FOR)) { //Support added for the forLoop block in parser
            ...
            if (!see(SEMI)) {
                ...
                mustBe(SEMI); //to parse the first condition if given and capture semicolon
            }
            if(!see(SEMI)) {
                expr = expression();
            }
            mustBe(SEMI); //to parse the second condition if given and capture semicolon

            if(!see(RPAREN)) {
                forUpdate = forUpdate(); //to parse the incremental condition if given and capture
                // RPAREN
            }
            mustBe(RPAREN);
            JStatement body = statement();
            return new JForStatement(line, forInit, expr, forUpdate, body);

        } else if (have(BREAK)) { //Support added for the BREAK Statement in parser
            mustBe(SEMI);
            return new JBreakStatement(line);
        } else if (have(CONTINUE)) { //Support added for the CONTINUE Statement in parser
            mustBe(SEMI);
            return new JContinueStatement(line);

        }  else if (have(SWITCH)) { //Support added for the SWITCH Statement in parser
        ...
        } else if (have(TRY)) { //Support added for the TRY - CATCH - FINALLY Block Statement in
            // parser
        ...
        }  else if (have(THROW)) { //Support added for the THROW Statement in parser
            JExpression expr = expression();
            mustBe(SEMI);
    ...
    /**
     * Parses and returns a list of forInit statement expression
     *
     * <pre>
     ...
     * </pre>
     *
     * @return a list of forInit statement expression.
     */
    ...
    /**
     * Parses and returns a list of forUpdate statement expression
     *
     * <pre>
     *   forUpdate  ::= statementExpression { COMMA statementExpression }
     * </pre>
     *
     * @return a list of forUpdate statement expression.
     */
    ...
    /**
     * Parses and returns a list of switch block statement group
     *
     * <pre>
     *   switchBlockStatementGroup  ::= switchLabel { switchLabel } { blockStatement }
     * </pre>
     *
     * @return a list of switch block statement group.
     */
    ...
    /**
     * Parses and returns a list of switch label expressions.
     *
     * <pre>
     ...
     * </pre>
     *
     * @return a list of switch label expressions.
     */
    ...
    private JStatement statementExpression() {
        ...
        if (expr instanceof JAssignment
                ...
                || expr instanceof JPostIncrementOp // added support for ++
                || expr instanceof JPreDecrementOp // added support for --
     ...
     *                                [ ( ASSIGN | PLUS_ASSIGN | DIV_ASSIGN  ) assignmentExpression ]
    ...
    private JExpression conditionalExpression() {
        int line = scanner.token().line();
        JExpression conditionalExpression = conditionalOrExpression();
        if (have(QUESTION)) {
            JExpression trueExpression = expression();
            mustBe(COLON);
            JExpression falseExpression = conditionalExpression();
            conditionalExpression = new JConditionalExpression(line, conditionalExpression, trueExpression, falseExpression);
        }
    ...
    private JExpression conditionalOrExpression() {
        ...
        while (more) {
            //Added Logical AND shift op "||" for parser to parse
            if (have(LOR)) {
     ...
     *
     * <pre>
     *   conditionalAndExpression ::= inclusiveORExpression { OR inclusiveORExpression }
     * </pre>
     *
     * @return an AST for a conditional-and expression.
     */
    ...
    /**
     * Parses an Inclusive OR Operator expression and returns an AST for it.
     *
     * <pre>
     *   inclusiveORExpression ::= exclusiveORExpression { LOR exclusiveORExpression }
     * </pre>
     *
     * @return an AST for Inclusive OR (||) and operator expression.
     */
    private JExpression inclusiveORExpression() {
        ...
        while (more) {
            //Added inclusive shift op "|" for parser to parse
            if (have(OR)) {
    ...
    /**
     * Parses an exclusive OR Operator expression and returns an AST for it.
     *
     * <pre>
     *   exclusiveORExpression ::= andOPExpression { XOR andOPExpression }
     * </pre>
     *
     * @return an AST for Exclusive OR (^) operator expression.
     */
    private JExpression exclusiveORExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = andOPExpression();
        while (more) {
            //Added Exclusive OR shift op "^" for parser to parse
            if (have(XOR)) {
                lhs = new JXorOp(line, lhs, andOPExpression());
            }  else {
                more = false;
            }
        }
    ...
    /**
     * Parses an AND Operator expression and returns an AST for it.
     *
     * <pre>
     *   andOPExpression ::= equalityExpression { AND equalityExpression }
     * </pre>
     *
     * @return an AST for an and (&) operator expression.
     */
    private JExpression andOPExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = equalityExpression();
        while (more) {
            //Added AND shift op "&" for parser to parse
            if (have(AND)) {
    ...
    private JExpression relationalExpression() {
        int line = scanner.token().line();
        JExpression lhs = aShiftExpression();
        if (have(GT)) {
            return new JGreaterThanOp(line, lhs, aShiftExpression());
        } else if (have(GE)) {
            return new JGreaterEqualOp(line, lhs, aShiftExpression());
        }else if (have(LE)) {
            return new JLessEqualOp(line, lhs, aShiftExpression());
        } else if (have(LT)) {
            return new JLessThanOp(line,lhs, aShiftExpression());
        }else {
            return lhs;
        }
    }
    ...
    /**
     * Parses an aShiftExpression expression and returns an AST for it.
     *
     * <pre>
     *   aShiftExpression ::= additiveExpression
     *                              { ( ALSHIFT | ARSHIFT | LRSHIFT ) additiveExpression }
     * </pre>
     *
     * @return an AST for an additive expression.
     */
    private JExpression aShiftExpression() {
        int line = scanner.token().line();
        boolean more = true;
        JExpression lhs = additiveExpression();
        while (more) {
            //Added Left shift op "<<" for parser
            if (have(ALSHIFT)) {
                return new JALeftShiftOp(line, lhs, additiveExpression());
                //Added Right shift op ">>" for parser
            } else if (have(ARSHIFT)) {
                return new JARightShiftOp(line, lhs, additiveExpression());
                //Added Logical Right shift op ">>>" for parser
    ...
    private JExpression multiplicativeExpression() {
        ...
        while (more) {
            ...
            } // Added support for DIVIDEND "/" operator for parser
            else if (have(DIV)){
                lhs = new JDivideOp(line, lhs, unaryExpression());
            }// Added support for REMAINDER "%" operator for parser
            else if (have(REM)){
    ...
    private JExpression unaryExpression() {
        ...
        if (have(INC)) {
            ...
            // Introduced UNARY_PLUS for Arithmatic Operations
        } else if (have(PLUS)) {
            return new JUnaryPlusOp(line, unaryExpression());
            // Introduced UNARY_COMPLEMENT for BitWise Not Operations
        } else if (have(NOT)) {
    ...
    private JExpression simpleUnaryExpression() {
        int line = scanner.token().line();
        if (have(LNOT)) {
            return new JLogicalNotOp(line, aShiftExpression());
        }
        else if (seeCast()) {
            mustBe(LPAREN);
