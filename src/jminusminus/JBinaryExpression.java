...
class JMultiplyOp extends JBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (lhs.type() == Type.INT && rhs.type() == Type.INT) {
            type = Type.INT;
        } else if (lhs.type() == Type.LONG && rhs.type() == Type.LONG) { // support for long
            type = Type.LONG;
        } else if (lhs.type() == Type.DOUBLE && rhs.type() == Type.DOUBLE) { // support for Double
            type = Type.DOUBLE;
        }
        return this;
    }
    ...
    public void codegen(CLEmitter output) {
        ...
        } else if (lhs.type() == Type.LONG) { // support for long
            output.addNoArgInstruction(LMUL);
        } else if (lhs.type() == Type.DOUBLE) { // support for Double
...
class JRemainderOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        ...
        } // support added for remainder op
    }
}
...
class JOrOp extends JBinaryExpression {
    ...
    public JExpression analyze(Context context) {
        lhs = (JExpression) lhs.analyze(context);
        rhs = (JExpression) rhs.analyze(context);
        lhs.type().mustMatchExpected(line(), Type.INT); // type is INT on both sides lhs & rhs
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IOR); //Support for inclusive or op
    }
}
...
class JXorOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IXOR); //Support added for exclusive or op
    }
}
...
class JAndOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IAND); //support for AND & op
    }
}
...
class JALeftShiftOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(ISHL); //support for arithmatic left shift op
    }
}
...
class JARightShiftOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(ISHR); // support for arithmatic right shift op
    }
}
...
class JLRightShiftOp extends JBinaryExpression {
    ...
    public void codegen(CLEmitter output) {
        lhs.codegen(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IUSHR); //support for logical right shift op
    }
}
