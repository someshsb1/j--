...
class JConstructorDeclaration extends JMethodDeclaration implements JMember {
    ...
    public void preAnalyze(Context context, CLEmitter partial) {
        ...
        if (exceptions != null) {
            ...
            for (int i = 0; i < exceptions.size(); i++) {
                exceptions.set(i, (TypeName) exceptions.get(i).resolve(context));
    ...
    public JAST analyze(Context context) {
        ...
        for (JFormalParameter param : params) {
            ...
            //constructor declaration support to skip and offset for long and double
            if (param.type() == Type.DOUBLE || param.type() == Type.LONG) { this.context.nextOffset(); }
            defn.initialize();
            this.context.addEntry(param.line(), param.name(), defn);
        }

        //During the analysis of JConstructorDeclaration, convert the list of exceptions (stored as TypeName objects)
