...
class JPlusAssignOp extends JAssignment {
    ...
    public JExpression analyze(Context context) {
        ...
        if (lhs.type().equals(Type.INT)) {
            ...
            type = Type.INT;
        } else if (lhs.type().equals(Type.LONG)) { // operand support for long
            rhs.type().mustMatchExpected(line(), Type.LONG);
            type = Type.LONG;
        } else if (lhs.type().equals(Type.DOUBLE)) {
            rhs.type().mustMatchExpected(line(), Type.DOUBLE); //// operand support for double
            type = Type.DOUBLE;
    ...
    public void codegen(CLEmitter output) {
        ...
        if (lhs.type().equals(Type.STRING)) {
            rhs.codegen(output);
        } else if (lhs.type().equals(Type.LONG)) { // operand output support for long
            ((JLhs) lhs).codegenLoadLhsRvalue(output);
            rhs.codegen(output);
            output.addNoArgInstruction(LADD);
        } else if (lhs.type().equals(Type.DOUBLE)) { // operand output support for double
            ((JLhs) lhs).codegenLoadLhsRvalue(output);
            rhs.codegen(output);
...
class JMinusAssignOp extends JAssignment {
    ...
    public JExpression analyze(Context context) {
        ...
        if (lhs.type().equals(Type.INT)) {
            ...
            rhs.type().mustMatchExpected(line(), Type.LONG); // operand support for long
            type = Type.LONG;
        } else if (lhs.type().equals(Type.DOUBLE)) {
            rhs.type().mustMatchExpected(line(), Type.DOUBLE); // operand support for double
            type = Type.DOUBLE;
...
class JOrAssignOp extends JAssignment {
    ...
    public JExpression analyze(Context context) {
        ...
        rhs = (JExpression) rhs.analyze(context);
        rhs.type().mustMatchExpected(line(), Type.INT); // operand support, type is Int
        type = Type.INT;
        return this;
    }
    ...
    public void codegen(CLEmitter output) {
        ...
        ((JLhs) lhs).codegenLoadLhsRvalue(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IOR); // support for IOR
        if(!isStatementExpression) {
            ((JLhs) lhs).codegenDuplicateRvalue(output);
        }
        ((JLhs) lhs).codegenStore(output);
...
class JAndAssignOp extends JAssignment {
    ...
    public JExpression analyze(Context context) {
        ...
        rhs = (JExpression) rhs.analyze(context);
        rhs.type().mustMatchExpected(line(), Type.INT); //type is Int
        type = Type.INT;
        return this;
    }
    ...
    public void codegen(CLEmitter output) {
        ...
        ((JLhs) lhs).codegenLoadLhsRvalue(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IAND); // support for IAND operand
        if(!isStatementExpression) {
            ((JLhs) lhs).codegenDuplicateRvalue(output);
        }
        ((JLhs) lhs).codegenStore(output);
...
class JXorAssignOp extends JAssignment {
    ...
    public void codegen(CLEmitter output) {
        ...
        ((JLhs) lhs).codegenLoadLhsRvalue(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IXOR); //support codegen output for IXOR operand
        if(!isStatementExpression) {
            ((JLhs) lhs).codegenDuplicateRvalue(output);
        }
        ((JLhs) lhs).codegenStore(output);
...
class JALeftShiftAssignOp extends JAssignment {
    ...
    public void codegen(CLEmitter output) {
        ...
        ((JLhs) lhs).codegenLoadLhsRvalue(output);
        rhs.codegen(output);
        output.addNoArgInstruction(ISHL); // support codegen output for ISHL operand
        if(!isStatementExpression) {
            ((JLhs) lhs).codegenDuplicateRvalue(output);
        }
        ((JLhs) lhs).codegenStore(output);
...
class JARightShiftAssignOp extends JAssignment {
    ...
    public void codegen(CLEmitter output) {
        ...
        ((JLhs) lhs).codegenLoadLhsRvalue(output);
        rhs.codegen(output);
        output.addNoArgInstruction(ISHR); //codegen instruction support added for ISHR operand
        if(!isStatementExpression) {
            ((JLhs) lhs).codegenDuplicateRvalue(output);
        }
        ((JLhs) lhs).codegenStore(output);
...
class JLRightShiftAssignOp extends JAssignment {
    ...
    public void codegen(CLEmitter output) {
        ...
        ((JLhs) lhs).codegenLoadLhsRvalue(output);
        rhs.codegen(output);
        output.addNoArgInstruction(IUSHR); //codegen output support added for IUSHR operand
        if(!isStatementExpression) {
            ((JLhs) lhs).codegenDuplicateRvalue(output);
        }
        ((JLhs) lhs).codegenStore(output);
