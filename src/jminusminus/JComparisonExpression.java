...
class JGreaterEqualOp extends JComparisonExpression {
     ...
     */
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        // Generate code to load the left-hand side onto the stack

        lhs.codegen(output);
        // Generate code to load the right-hand side onto the stack

        rhs.codegen(output);
        // Generate code for the comparison, branching to the target label if the
        // comparison is true (onTrue is true) or false (onTrue is false)
        output.addBranchInstruction(onTrue ? IF_ICMPGT : IF_ICMPLE, targetLabel);
    }
}
...
class JLessThanOp extends JComparisonExpression {
    ...
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        ...
        if (type == Type.BOOLEAN) {
            ...
            } else if (lhs.type() == Type.DOUBLE) { // operand support for double
                output.addNoArgInstruction(DCMPG);
                output.addBranchInstruction(onTrue ? IF_ICMPEQ : IFGE, targetLabel);
            } else if (lhs.type() == Type.LONG) { // operand support for long
