...
class JNegateOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (operand.type() == Type.INT) {
            ...
            type = Type.INT;
        } else if (operand.type() == Type.LONG) { //support for Long
            operand.type().mustMatchExpected(line(), Type.LONG);
            type = Type.LONG;
        } else if (operand.type() == Type.DOUBLE) { //support for Double
    ...
    public void codegen(CLEmitter output) {
        ...
        } else if (operand.type() == Type.LONG) { // output support for Long
            output.addNoArgInstruction(LNEG);
        } else if (operand.type() == Type.DOUBLE) { //output support for Double
...
class JUnaryPlusOp extends JUnaryExpression {
     ...
     */
    public void codegen(CLEmitter output) {
        // IADD not required as this is only UNARY_PLUS Operations
        operand.codegen(output);
    }
}
...
class JComplementOp extends JUnaryExpression {
    ...
    public void codegen(CLEmitter output) {
        operand.codegen(output);
        // computed as the “exclusive or” operand and -1 as there is no instructions for unary
        // complement in j--
        output.addNoArgInstruction(ICONST_M1); // -1
        output.addNoArgInstruction(IXOR);   // exclusive OR
...
class JPostIncrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(operand instanceof JLhs)) {
            JAST.compilationUnit.reportSemanticError(line, "Operand to ++ (post) must have an " +
                    "LValue" +
                    ".");
            type = Type.ANY;
        } else {
...
class JPreDecrementOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        if (!(operand instanceof JLhs)) {
            JAST.compilationUnit.reportSemanticError(line, "Operand to -- (pre) must have an " +
                    "LValue.");
            type = Type.ANY;
        } else {
            operand = operand.analyze(context);
